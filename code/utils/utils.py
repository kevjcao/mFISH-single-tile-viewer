...
def create_channel_control_box(ch, defaults, gene, width="400px"):
    ...
    slider_vmin = widgets.FloatSlider(
        ...
        description='',
        continuous_update=False,
        layout=widgets.Layout(width='80%', margin='2px')
    )
    text_vmin = widgets.FloatText(
        value=def_vmin,
        layout=widgets.Layout(width='20%', margin='2px')
    )
...
def update_channel_plot(ch, store, res_level, z_center, z_margin, vmin, vmax, color, gene):
...
def create_round_interactive_view(round_idx, main_channel, channels_per_round, genes_list, res_level, zarr_paths, default_z_center, default_z_margin):
    ...
    Displays global Z-controls and builds brightness sliders, text inputs, and a color picker for the image.
    """
    store = zarr.open(zarr_paths[round_idx], mode='r')
    round_channels = channels_per_round[round_idx]
    gene_dict = genes_list[round_idx]
    
    # get total z-planes using from the main ch
    ...
    # global Z-controls
    slider_z_center, slider_z_margin, global_z_box = create_global_z_controls(total_z, default_z_center, default_z_margin, width="20%")
    
    # controls for each ch
    channel_boxes = []
    for ch in round_channels:
        ...
        out = widgets.interactive_output(
            lambda vmin, vmax, z_center, z_margin, color, ch=ch, gene=gene:
                update_channel_plot(ch, store, res_level, z_center, z_margin, vmin, vmax, color, gene),
            {'vmin': slider_vmin, 'vmax': slider_vmax, 'z_center': slider_z_center, 'z_margin': slider_z_margin, 'color': color_picker}
        )
        box = widgets.VBox([control_box, out], layout=widgets.Layout(margin='2px', padding='2px', width="600px"))
    ...
    display(global_z_box, channels_hbox)
